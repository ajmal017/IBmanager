...
import sys
sys.path.append('/Users/joanarau-schweizer/PycharmProjects')
...
print('Ticker: ',ticker, ' Spot: ',spot,' Div: ' ,div)
...
delta = datetime.datetime.strptime(exp, '%Y%m%d') - datetime.datetime.strptime(t0, '%Y%m%d')
delta_annual = delta.days / 365


if tst == False:
    import option_chain_get as chain
    ch = chain.get_chain(ticker,[exp])
    ch = ch[exp]

else:

    ch = pd.read_csv('/Users/joanarau-schweizer/PycharmProjects/IBmanager/db/test_option_chain/'+exp)


def calc_put_spreads(ch=ch,spot=spot,div=div,r=r,t0=t0,exp=exp,kind = 'P',min_otm=min_otm,min_pop=min_pop,max_point_spread=max_point_spread):
...
def calc_call_spreads(ch=ch,spot=spot,div=div,r=r,t0=t0,exp=exp,kind = 'C',min_otm=min_otm,min_pop=min_pop,max_point_spread=max_point_spread):
